<template>
  ...
  <div class="item" v-for="(item,index) in orderData" :key="index" >
    <div class="logos"><van-tag size="large" round type="danger" color="">微商城自营店</van-tag><div class="success">{{item.type=='2'?'等待付款':'交易成功'}}</div></div>
    <div class="shopList" @click="navigateDetail(item1.detailId)"  v-for="item1 in item.orderList" :key="item1.orderId" >
      ...
      <div class="right">
        <div class="title">{{item1.title}}/{{item1.descs}}</div> 
    ...
    <div class="btns">
      ...
      <van-button  @click="pay(item.orderNumber)" size="small" style="margin-left:.825rem;" v-if="item.type=='2'"  type="danger" round>立即支付</van-button>
    </div>
  </div>

<template>
  ...
  <div class="checkoutbox">
        ...
        <div  v-for="item in defaultAddress" :key="item.addressId" class="top">
          <div class="left">
            ...
            <div class="info" @click="showPopupClick">
              ...
              <div class="phone"><span>{{item.name}}</span><span>{{item.phone}}</span></div>
            </div>
          </div>
          <div class="right">
    ...
    <div class="footer">
      <div class="·">
        <div>共计 <span style="font-weight: bold;">{{totalNum}} </span> 商品</div>
        <div class="total"><span>合计:</span><span style="font-weight: bold;color:red;font-size: 1.4625rem;margin-right: .625rem;">￥{{totalPrice}}</span></div>
      </div>
      <div class="right">
      ...
      <div class="popup">
        <div class="contents">
          <div class="list" v-for="item in allAddress"  :key="item.addressId">
            ...
            <div class="centents" @click="clickSetDefaultAddress(item.addressId)" >
              <div class="infos"><span class="name">{{item.name}}</span>
                <span class="phones">{{item.phone}}</span>
                <van-tag plain v-if="item.type=='1'" type="danger">默认</van-tag>
              </div>
...
<van-dialog use-slot title="请扫码支付"  v-model:show="payDialogShow" show-cancel-button  @confirm="confirmPay" confirmButtonspan="我已支付" @cancel="cancelPay">
...
<script setup lang="ts">
  import {getSelectConfirmOrder,getDeleteOrder,getAllAddress,setDefaultAddress,setOrderState,setOrderAddressByPay} from "@/api/request"
...
const submitOrder= async()=>{
  ...
  const list:ICarOrderData[] = JSON.parse(qr)
    ...
    const {data:res} = await setOrderAddressByPay(orderNumber.value,defaultAddress.value[0]?.addressId,carIdList,"2")

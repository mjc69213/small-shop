 ...
 <div class="car">
  <div class="content">
    <van-swipe-cell   v-for="item in carOrderData" :key="item.detailId"  >
              <div class="item" >
                <div class="tops">
                  <div class="tops_left"><van-checkbox checked-color="#ff0000" :name="213" v-model="item.checked" data-carId="{{item.carId}}" @change="onChangeCheck(item.carId,item.checked)"></van-checkbox></div>
                  <div class="right"><van-tag size="large" round type="danger" color="#cf0a2c" >微商城自营店</van-tag></div>
                </div>
                <div class="center" >
                  <img @click="navigateDetail(item.detailId)" :src="item.titleImg" />
                  <div class="center_right">
                    <div @click="navigateDetail(item.detailId)" class='title'>
                      <span class="tag" v-if="item1.tag">{{item1.tag}}</span>
                      {{item.title}}
                    </div>
                    ...
                    <div class='price'>
                      ...
                      <div>
                        <van-stepper cf0a2c  v-model="item.num" max="6" :name="item.carId"  theme="round" button-size="22" disable-input  @change="onChangeNum" />
                      </div>
                    </div>
                  </div>
                </div>
            ...
            <template #right>
              <div  @click="clickDelete(item.carId)"  class="delete" bindtap="deleteCar" >删除</div>
            </template>
    </van-swipe-cell>
...
<div class="haha"> 
  <div class="left">
    <van-checkbox checked-color="#cf0a2c" v-model="allChecked" data-carId="{{allChecked}}" @change="allSelectChecked">全选</van-checkbox>
  </div>
  <div class="right">
    <div class="right_top">
    ...
    <van-button icon="balance-pay" type="danger" @click="submitOrderClick" round color="#cf0a2c">去结算</van-button>
  </div>
...
<script setup lang="ts">
  import {getSelectCarShop,updateCarNum,removeCarShop,submitOrder} from "@/api/request"
...
const submitOrderClick = async()=>{
  ...
  const orderData:ICarOrderData[] = carOrderData.value?.filter(item=>item.checked)
...
const onChangeCheck = async(carId: any,value:any)=>{
  ...
  if(res){
    ...
    // console.log("有未选的",carId,value)
  }
  
}

const totalNum = computed(()=>{
  const t = carOrderData.value?.filter(item=>item.checked)
  .reduce((total,item)=>total+=item.num,0)
  return t
})
const totalPrice = computed(()=>{
  return carOrderData.value?.filter(item=>item.checked)

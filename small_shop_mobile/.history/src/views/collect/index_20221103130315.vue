<template>
  ...
  <div class="contents">
    <div class="items" v-for="item in collectData" :key="item.detailId">
      <div class="logos"><van-tag size="large" round type="danger">微商城自营店</van-tag></div>
    <div class="list" >
      ...
      <div class="right" @click="navigateDetail(item.detailId)" >
        ...
        <div class="bottom"><span>￥{{item.price}}</span><span>￥{{item.prePrice}}</span></div>
      </div>
    </div>
    <div class="btns">
      <van-button size="small" @click="cancelCollect(item.collectId)"  round>取消收藏</van-button>
      <van-button @click="addCar(item.detailId)" size="small" type="danger" round>加入购物车</van-button>
    </div>
      </div>
  ...
  .contents{
    ...
    .likes {
      ...
      .tip {
        ...
        background-image: linear-gradient(to right bottom, var(--themeColor),
            rgb(228, 52, 219),
            black,
            orange);

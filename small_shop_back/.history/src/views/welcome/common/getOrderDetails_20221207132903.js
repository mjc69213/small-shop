...
const getOrder = async () => {
  ...
  const awaitCar = res.data?.filter(item =>item.type=='1' ) || 0//加入购物车
  const awaitPay = res.data?.filter(item =>item.type=='2' )  || 0 //待付款
  const awaitSend = res.data?.filter(item =>item.type=='3' ) || 0 //待发货

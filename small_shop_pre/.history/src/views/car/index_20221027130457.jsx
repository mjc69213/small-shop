...
import { setSuccessTip, setPayDialog, setCarTotal, setRemoveCar } from '../../store/reducer/global';
export default function Car() {
  ...
  const columns = [
    ...
    },
    {
      ...
      dataIndex: 'num',
      width: 120,
      render: (a, b) => (<Input disabled={disableNum} type='number' defaultValue={a} min={1} max={6} onChange={(e) => changeNumber(e, b.carId)} ></Input>),
  ...
  const changeNumber = async (e, carId) => {
    setDisableNum(true)
    const obj = { userId, num: e.target.value, carId }
  ...
  const getCarData = async () => {
    setLoading(true)
    const { data: res } = await selectCar(userId)
    if (res.code != 200) return setCarData([]).finally(()=>{
      setTimeout(() => {
        setLoading(false)
      }, 500);
    })
  ...
  return (
    <div className={style.box}>
      <div className={style.car}>
        ...
        <h1 >我的购物车</h1>
        {carData.length > 0 && <Table rowSelection={rowSelection} columns={columns} rowKey="carId" dataSource={carData} />}
      </div>
      {loading &&(
        <div className={style.loading} style={{padding:loading?'160px 0':'0'}}>
        <Spin />
      </div>
      )}

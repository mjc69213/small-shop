...
export default function Details() {
  ...
  const [loading, setIsCollect] = useState(true)
  useEffect(() => {
  ...
  const getDetails = async (detailId) => {
    const { data: res } = await getShopdetails(detailId)
  ...
  if (shopDetail.length) {
    var { descs, detailDesc, prePrice, price, title, titleImg } = shopDetail[0]
  }
    ...
    <div className={style.box}>
      ...
      <div className={style.details}>
        <div className={style.top}>
          <li className={style.topLeft}>{title}</li>
        ...
        <div className={style.center}>
          ...
          <div className={style.centerRight}>
            ...
            <p className={style.price}><span>{price}元</span><span>{prePrice}元</span></p>
            <p className={style.line}></p>
            <div className={style.total}>
              <div className={style.totalTop}>
                <p className={style.totalTopTitle}>{title}</p>
                <p className={style.totalTopPrice}>
                  <span>{price}元</span>
                  <span>{prePrice}元</span>
                </p>
              </div>
            ...
            <div className={style.btns} >
              ...
              <div className={`${style.like} ${!isCollect?'':style.collectActive}`} onClick={() => clickCollect(detailId)} >{isCollect ? "取消收藏" : "收藏"}</div>
            </div>
